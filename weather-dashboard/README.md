...
Open-Meteo Weather APIは**緯度・経度でしか天気を取得できない**仕様のため、都市名から天気を取得するには2段階のフェッチが必要です。

```
ユーザー入力: "Tokyo"
        ↓
[1. Geocoding API] 都市名 → 緯度経度に変換
        ↓
[2. Weather API] 緯度経度 → 天気データを取得
```

**この設計のメリット:**

- **精度**: 緯度経度は曖昧さがない（「Paris」はフランス？テキサス？）
- **柔軟性**: 地図クリックや現在地取得など、都市名以外の入力にも対応可能
